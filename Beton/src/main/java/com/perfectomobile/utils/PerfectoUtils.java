...
public class PerfectoUtils {
	 ...
	 * @param retries
	 *            the integer number retries
	 * @param retryIntervalSeconds
	 *            the integer number of retry interval seconds
	 * @return the driver
	 */
	public static RemoteWebDriver getDriver(DesiredCapabilities cap, int retries, int retryIntervalSeconds) {
		...
		do {
			try {
				...
				index--;
				System.out.println(
						"device wasn't allocated successfully: " + cap.toString() + "\n Retries left: " + index);
	 ...
	 * checkVisual
	 * <ul>
	 * <li>Performs a visual context evaluation to identify the existence of a
	 * text string on a page
	 * <ul>
	 *
	 * @param driver
	 *            the driver
	 ...
	 * <li>Looks for an element identifier locator defined as a By type for an
	 * overall period of time define by the timeout, polling every 250 ms
	 * <li>Returns the WebElement once found or null if not found inside timeout
	 * limit
	 * <ul>
	 * 
	 * @param locator
	 ...
	 * 
	 * @param format
	 *            The format of the requested response. e.g.
	 *            "yyyy-MM-dd HH-mm-ss"
	 * @param offset
	...
	/**
	 * *************************************************************************
	 * ** gets a specific device property out of the Dictionary. returns the
	 * value of the property for example: getDeviceProperty("Model") will return
	 * the model of the device(ie iPhone-6)
	 * *************************************************************************
	 * ***
	 *
	 * @param driver
	 ...
	 * <ul>
	 * <li>Uses provided key to decrypt provided message encrypted using the
	 * same key
	 * <ul>
	 * 
	 * @author Brian Clark
	...
	// * <ul>
	// * <li>deviceId; manufacturer; model; distributer; description; firmware;
	// imsi; wifiMacAddress;
	// * link; operator; phoneNumber; location; lastCradleId; language; status;
	// mode; available;
	// * reserved; inUse; allocatedTo; operabilityRating; cradleId; position;
	// os; osVersion;
	...
	// * @return the device property
	// */
	// public static String getDeviceProperty(RemoteWebDriver driver, Properties
	// property){
	//
	...
	// String deviceID =
	// driver.getCapabilities().asMap().get("deviceName").toString();
	// String res = null;
	// try {
	// URL url = new
	// URL("https://"+host+"/services/handsets/"+deviceID+"?operation=info&user="+user+"&password="+pwd);
	// //make connection
	...
	public static String uploadLocalFileToMediaRepo(RemoteWebDriver driver, String file, String repoPath) {
		...
		try {
			...
			URL url = new URL("https://" + host + "/services/repositories/media/" + repoFile + "?operation=upload&user="
					+ user + "&password=" + pwd);
	 ...
	 * The Enum MatchMode.
	 */
	public static enum MatchMode {// all caps --
									// contain,equal,startwith,endwith,first,last,index
	 ...
	 * The Enum ScrollNext.
	 */
	public static enum ScrollNext {// all caps --
